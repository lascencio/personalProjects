...
Public Class dalCONCAR_Anexo
    ...
    Private Shared Function ExisteProyecto(ByVal pAnexo As String) As Boolean
        If Not String.IsNullOrEmpty(pAnexo) Then
            Dim MySqlString As String = "SELECT COUNT(*) FROM CON.ANEXO_PROYECTO_CONCAR WHERE acodane=@vAnexo "
            Using MySQLDbconnection As New SqlConnection(myConnectionStringSQL_Repository)
                Dim MySQLCommand As New SqlCommand(MySqlString, MySQLDbconnection)
                MySQLCommand.Parameters.AddWithValue("vAnexo", pAnexo)
                MySQLDbconnection.Open()
    ...
    Public Shared Function Obtener(ByVal pAnexo As String, ByVal pAnexoTipo As String) As entCONCAR_Anexo
        If Existe(pAnexo, pAnexoTipo) Then
            CadenaSQL = "SELECT avanexo, acodane, adesane, arefane, aruc, acodmon, aestado, adate, ahora, SPACE(1) AS avrete, 0 AS aporre " & _
                        "FROM CON.ANEXOS_DETALLE WHERE avanexo='" & pAnexoTipo & "' AND acodane='" & pAnexo & "'"
            Return ObtenerPersonal(New entCONCAR_Anexo, CadenaSQL)
        Else
            Return New entCONCAR_Anexo
        End If
    End Function

    Public Shared Function ObtenerPersonal(ByVal pAnexo As String, ByVal pAnexoTipo As String) As entCONCAR_Anexo
        If Existe(pAnexo, pAnexoTipo) Then
            CadenaSQL = "SELECT avanexo, acodane, adesane " & _
                        "FROM CON.ANEXOS_DETALLE WHERE avanexo='" & pAnexoTipo & "' AND acodane='" & pAnexo & "'"
            Return ObtenerPersonal(New entCONCAR_Anexo, CadenaSQL)
        Else
            Return New entCONCAR_Anexo
    ...
    Public Shared Function ObtenerCliente(ByVal pAnexo As String) As entCONCAR_Anexo
        If ExisteCliente(pAnexo) Then
            CadenaSQL = "SELECT avanexo, acodane, adesane, arefane, aruc, acodmon, aestado, adate, ahora, avrete, aporre " & _
                        "FROM CON.ANEXOS_DETALLE WHERE avanexo='C' AND acodane='" & pAnexo & "' "
            Return ObtenerAnexo(New entCONCAR_Anexo, CadenaSQL)
        Else
            Return New entCONCAR_Anexo
        End If
    End Function
    ...
    Private Shared Function ObtenerAnexo(ByVal cAnexo As entCONCAR_Anexo, ByVal MyStringSQL As String) As entCONCAR_Anexo
    ...
    Private Shared Function ObtenerPersonal(ByVal cAnexo As entCONCAR_Anexo, ByVal MyStringSQL As String) As entCONCAR_Anexo
        Dim MyDT As New dsCONCAR_Anexos.ANEXODataTable

        Call ObtenerDataTableSQL(MyStringSQL, MyDT, myConnectionStringSQL_Repository)
