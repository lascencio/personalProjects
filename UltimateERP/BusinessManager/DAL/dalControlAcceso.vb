...
Public Class dalControlAcceso
    ...
    Public Shared Function ObtenerAcceso(ByVal Empresa As String, ByVal Usuario As String, ByVal Nivel_1 As String, ByVal Nivel_2 As String, ByVal Nivel_3 As String) As dsControlAcceso.CONTROL_ACCESODataTable
        ...
        MystringSql = "SELECT * FROM GEN.CONTROL_ACCESO " & _
    ...
    Public Shared Function ObtenerAccesos(ByVal Usuario As entUsuario) As dsControlAcceso.CONTROL_ACCESO_LISTADataTable
        Dim MyDT As New dsControlAcceso.CONTROL_ACCESO_LISTADataTable
        If Existe(Usuario) Then
            MystringSql = "SELECT EMPRESA, USUARIO, NIVEL_1, NIVEL_2, NIVEL_3, PERMITIR_IMPRIMIR, " & _
                          "CASE PRIVILEGIOS WHEN 1 THEN 'CONTROL TOTAL' WHEN 2 THEN 'LECTURA/ESCRITURA' ELSE 'SOLO LECTURA' END AS PRIVILEGIOS, " & _
                          "PRIVILEGIOS AS PRIVILEGIOS_CODIGO " & _
                          "FROM GEN.CONTROL_ACCESO " & _
    ...
    Private Shared Function Existe(ByVal Empresa As String, ByVal Usuario As String, ByVal Perfil As String) As Boolean
        MystringSql = "SELECT COUNT(*) FROM GEN.CONTROL_ACCESO WHERE Empresa=@empresa AND Usuario=@usuario"
        Using MySQLConnection As New SqlConnection(myConnectionStringSQL_Repository)
            MySQLCommand = New SqlCommand(MystringSql, MySQLConnection)
    ...
    Private Shared Function ExisteAcceso(ByVal ControlAcceso As entControlAcceso) As Boolean
        MystringSql = "SELECT COUNT(*) FROM GEN.CONTROL_ACCESO WHERE Empresa=@empresa AND Usuario=@usuario AND Nivel_1=@nivel_1 AND Nivel_2=@nivel_2 AND Nivel_3=@nivel_3"
        Using MySQLConnection As New SqlConnection(myConnectionStringSQL_Repository)
            MySQLCommand = New SqlCommand(MystringSql, MySQLConnection)
            With MySQLCommand.Parameters
    ...
    Private Shared Sub Actualizar(ByVal ControlAcceso As entControlAcceso)
        MystringSql = "UPDATE GEN.CONTROL_ACCESO " & _
        ...
        Using MySQLDbconnection As New SqlConnection(myConnectionStringSQL_Repository)
            ...
            MySQLCommand = New SqlCommand(MystringSql, MySQLDbconnection)
    ...
    Private Shared Sub Insertar(ByVal ControlAcceso As entControlAcceso)
        MystringSql = "INSERT INTO GEN.CONTROL_ACCESO " & _
    ...
    Private Shared Sub Eliminar(ByVal ControlAcceso As entControlAcceso)
        MystringSql = "DELETE FROM GEN.CONTROL_ACCESO " & _
                      "WHERE Empresa=@empresa AND Usuario=@usuario AND Nivel_1=@nivel_1 AND Nivel_2=@nivel_2 AND Nivel_3=@nivel_3"
