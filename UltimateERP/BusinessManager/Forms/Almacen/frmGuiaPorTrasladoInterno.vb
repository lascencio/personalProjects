Public Class frmGuiaPorTrasladoInterno
    ...
    Private MotivoTrasladoDescripcion As String = "TRASLADO ENTRE ALMACENES"
    Private TipoOperacion As String = "G2" ' G2 Translado entre almacenes, G4 - Venta Vehicular, G5 - Venta Directa
    ...
    Private Sub frmGuiaPorTrasladoInterno_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If MyDTAlmacenes.Rows.Count <> 0 Then
            For NEle As Integer = 0 To MyDTAlmacenes.Rows.Count - 1
                If MyDTAlmacenes(NEle).CODIGO <> MyUsuario.almacen Then
                    MyAlmacenDestino.ImportRow(MyDTAlmacenes(NEle))
                End If
            Next
        End If
    ...
    Private Sub cmbAlmacenDestino_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmbAlmacenDestino.SelectedIndexChanged
        ...
        Dim DomicilioDepartamento As String
        Dim DomicilioProvincia As String
        Dim DomicilioDistrito As String

        Dim AgenciaDestinoCodigo As String = "A0000"
        If cmbAlmacenDestino.SelectedIndex <> -1 Then
            AgenciaDestinoCodigo = AgenciaDestinoCodigo & cmbAlmacenDestino.SelectedValue
            AgenciaDestino = dalAgencia.Obtener(MyUsuario.empresa, AgenciaDestinoCodigo)

            DomicilioDepartamento = ObtenerDescripcion("_UBIGEO_REGION", AgenciaDestino(0).DEPARTAMENTO, True)
            DomicilioProvincia = ObtenerDescripcion("_UBIGEO_PROVINCIA", AgenciaDestino(0).PROVINCIA, True)
            DomicilioDistrito = ObtenerDescripcion("_UBIGEO", AgenciaDestino(0).UBIGEO, True)

            txtPuntoLlegada.Text = AgenciaDestino(0).DOMICILIO & " " & DomicilioDistrito & " " & DomicilioDepartamento & " " & DomicilioDepartamento
        Else
    ...
    Private Function CapturarEncabezadoGuia() As Boolean
        ...
        Else
            With MyGuia
                ...
                .motivo_traslado = "04" ' 01 - Venta, 04 - Traslado entre almacenes, 13 - Otros 
...
#Region "Botones"
    ...
    Private Sub UC_ToolBar_btnAceptar_Click() Handles UC_ToolBar.TB_btnAceptar_Click
        ...
        Else
            With MyOperacion
                ...
                .tipo_operacion = "11"   ' Traslado entre almacenes
    ...
    Private Sub UC_ToolBar_btnEditar_Click() Handles UC_ToolBar.TB_btnEditar_Click
        MyGuia = New entGuia
        Dim MyForm As New frmGuiaRemitenteLista(MyGuia, txtEjercicio.Text, txtMes.Text, "TODOS", "G2", "TODOS")
        MyForm.ShowDialog()
