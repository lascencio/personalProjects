...
function Enviar() {
    ...
    if (asunto == '') {
        ...
        if (confirm('¿Desea enviar esta proforma?')) {
            window.location.href = "/EstadoCuenta/EnviarEstadoCuenta?idGenerado=" + generado + "&cliente=" + cliente + "&asunto=" + asunto + "&cuerpo=" + cuerpo;
        }
    }
}

function VerPDF() {
    window.location.href = "/EstadoCuenta/DescargarPdf?liquidacion=" + generado;
}
function ConstruirTabla(result) {
    ...
    $("#tablaCorreo").append('<tr><th style="width:215px;">' + correo + '</th><td style="width:30px;"><a href="#"><img src=' + del + ' /></a></td></tr>');
    for (var i in result) {
        var usu = "'" + result[i] + "'";
